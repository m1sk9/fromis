...
const PREVIEW_DL_SIZE_LIMIT: u64 = 1 * 1024 * 1024;
...
impl GitHubPreview {
    pub fn is_exist(content: &str) -> bool {
        ...
        URL_REGEX.is_match(content) && !SKIP_URL_REGEX.is_match(content) && LINE_REGEX.is_match(content)
    }

    pub fn find_from_str(content: &str) -> anyhow::Result<Self> {
        let mut res = URL_REGEX.find_iter(content).flat_map(|m| {
            ...
            if segments.next()? != "blob" { // m1sk9/fromis/blob
                return None;
            }
            ...
            let ext = std::path::Path::new(filename).extension().map(|x| x.to_str().unwrap()).unwrap_or(""); // .rs
        ...
        res.pop().ok_or_else(|| anyhow::anyhow!("No GitHub URL found"))
    }
    ...
    pub async fn get_code(&self) -> anyhow::Result<String, GitHubPreviewError> {
        ...
        let res = reqwest::get(&raw_url).await.context("Failed to fetch raw content")?;
        ...
        Ok(content.lines().skip(skip).take(line2.saturating_sub(skip)).collect::<Vec<&str>>().join("\n"))
    }
}
